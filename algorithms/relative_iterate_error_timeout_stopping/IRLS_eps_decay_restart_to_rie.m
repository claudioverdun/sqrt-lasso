%% This is a version of IRLS algorithm with restarts for square-root LASSO which performs 
% iterations until time limit is reached or the two consecutive iterates are 
% close in terms of relative error.
%
...
%
% tol: >0, tolerance parameter for least squares solver, see help for lsqr
%           and pcg functions 
%
% N_it: integer >=0.  Number of IRLS iteration per restart.
%
...
function [x,time] = IRLS_eps_decay_restart_to_rie(A,b,lambda,eps0,x0,Nlsp,tol,N_it,decay,s, threshold, timeout)
% Initialization
time = 0;
tic;
...
if strcmp(solver,'cd')
    % For coordinate descent, some values can be precomputed and stored.
    nA = sum(abs(A).^2,1);
    nb = norm(b);
    res = A*x-b;
    res_norm = norm(res);
