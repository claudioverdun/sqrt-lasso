...
function [x, time] = proximal_newton_to_rie(A,b,eps1,lambda,x0, Ninner, threshold, timeout)
% Initialization
time = 0;
tic;
...
while true
    ...
    dt = toc;
    time = time + dt;
    [x_new, dt] = coordinate_descent(x, grad(x), H(x), x, lambda, Ninner, threshold, timeout -  time);
    tic 
    time = time + dt;
