...
for fi = 1:length(factors)
    ...
    %     lambda_opt = sqrt(m)*norminv(1 - 0.05/s);
        ...
        % % single run methods
        tic
        xr = ITEM_sqrtlasso(A,b,lambda,mu,L,eps1,eps2,x0,10^-4);
        times(1,fi,t) = toc;
        errors(1,fi,t) = norm(xr - x)/norm(x);
        tic
        xr2 = proximal_gradient(A,b,eps1,lambda,L,x0,10^-4);
        times(2,fi,t) = toc;
        errors(2,fi,t) = norm(xr2 - x)/norm(x);
        tic
        xr3 = proximal_newton(A,b,eps1,lambda,x0,1000,10^-4);
        times(3,fi,t) = toc;
        errors(3,fi,t) = norm(xr3 - x)/norm(x);
        tic
        xr4 = IRLS_v2(A,b,lambda,eps1,eps2,x0,10000,10^-4);
        times(4,fi,t) = toc;
