...
%         b = b + sigma*randn(size(b)); 
        ...
        % % single run methods
        [x_1, t_1] = IRLS_eps_decay_to(A,b,lambda,eps0_high,x0,10000,1000,'sqrt',s,'lsqr_fun', x, threshold, timeout);
        times(1,si,t) = t_1;
        ...
        [x_2, t_2] = IRLS_eps_decay_to(A,b,lambda,eps0_med,x0,10000,1000,'sqrt',s,'lsqr_fun', x, threshold, timeout);
        times(2,si,t) = t_2;
        ...
        [x_3, t_3] = IRLS_eps_decay_to(A,b,lambda,eps0_small,x0,10000,1000,'sqrt',s,'lsqr_fun', x, threshold, timeout);
        times(3,si,t) = t_3;
        ...
        [x_4, t_4] = IRLS_eps_decay_restart_to(A,b,lambda,eps0_high,x0,1000,1000,'sqrt',s,'lsqr_fun', x, threshold, timeout);
        times(4,si,t) = t_4;
        ...
        [x_5, t_5] = IRLS_eps_decay_restart_to(A,b,lambda,eps0_med,x0,1000,1000,'sqrt',s,'lsqr_fun', x, threshold, timeout);
        times(5,si,t) = t_5;
        ...
        [x_6, t_6] = IRLS_eps_decay_restart_to(A,b,lambda,eps0_small,x0,1000,1000,'sqrt',s,'lsqr_fun', x, threshold, timeout);
        times(6,si,t) = t_6;
        ...
        [x_7, t_7] = smooth_concomitant_lasso_v2_to(A, b, 10000, eps1, lambda, x0, x, threshold, timeout);
        times(7,si,t) = t_7;
...
legend({'high(-3)','med(-6)','small(-9)','restrat+high(-3)','restart+med(-6)','restart+small(-9)','concomitant lasso'})
