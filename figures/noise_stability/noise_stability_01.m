%% This script generates data for the upper half of Figure 2 in 
...
for snri = 1:length(SNR_level)
    ...
    for t=1:trials
        ...
        % Proximal gradient
        [x_2, t_2] = proximal_gradient_decay_to_rie(A,b,10^-1*eps1_huge,lambda,100/sqrt(eps1_huge),x0, threshold, timeout);
        times(2,snri,t) = t_2;
        ...
        [x_4, t_4] = smooth_concomitant_lasso_v2_decay_to_rie(A, b, 5, 10^-1*eps1_huge, lambda, x0, threshold, timeout);
        times(4,snri,t) = t_4;
...
writetable(results_err,'noise_stab_01_err.csv','Delimiter',' ')
