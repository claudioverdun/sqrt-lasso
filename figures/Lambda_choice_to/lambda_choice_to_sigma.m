...
ER = readtable('lambda_err.csv','Delimiter',' ');

figure
loglog(10.^Lmult*lambda_opt/1.5, [ER.irls_sigma avg_error(6,:).'])
...
writetable(ER,'lambda_err.csv','Delimiter',' ')

TI = readtable('lambda_time.csv','Delimiter',' ');
figure
loglog(10.^Lmult*lambda_opt/1.5, [TI.irls_sigma avg_time(6,:).'])
title('Runtime')
legend({'old sigma','new sigma'})

TI.irls_sigma = avg_time(6,:).';
writetable(TI,'lambda_time.csv','Delimiter',' ')

SFR = readtable('lambda_ratio.csv','Delimiter',' ');
figure
semilogx(10.^Lmult*lambda_opt/1.5,[SFR.irls_sigma avg_ratios(6,:).'])
title('Missing support percentage')
legend({'old sigma','new sigma'})

SFR.irls_sigma = avg_ratios(6,:).';
writetable(SFR,'lambda_ratio.csv','Delimiter',' ')

SFR2 = readtable('lambda_ratio2s.csv','Delimiter',' ');
figure
semilogx(10.^Lmult*lambda_opt/1.5,[SFR2.irls_sigma avg_ratio2s(6,:).'])
title('Missing support percentage (2s)')
legend({'old sigma','new sigma'})

SFR2.irls_sigma = avg_ratio2s(6,:).';
writetable(SFR2,'lambda_ratio2s.csv','Delimiter',' ')

% restart files

ER = readtable('lambda_err_restart.csv','Delimiter',' ');
ER.irls_sigma = avg_error(6,:).';
writetable(ER,'lambda_err_restart.csv','Delimiter',' ')

TI = readtable('lambda_time_restart.csv','Delimiter',' ');
TI.irls_sigma = avg_time(6,:).';
writetable(TI,'lambda_time_restart.csv','Delimiter',' ')

SFR = readtable('lambda_ratio_restart.csv','Delimiter',' ');
SFR.irls_sigma = avg_ratios(6,:).';
writetable(SFR,'lambda_ratio_restart.csv','Delimiter',' ')

SFR2 = readtable('lambda_ratio2s_restart.csv','Delimiter',' ');
SFR2.irls_sigma = avg_ratio2s(6,:).';
writetable(SFR2,'lambda_ratio2s_restart.csv','Delimiter',' ')
